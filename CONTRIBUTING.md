...
```bash
git clone https://github.com/YOUR_USERNAME/notifypro.git
cd notifypro
```
...
```bash
bench get-app notifypro /path/to/your/fork
bench --site dev.localhost install-app notifypro
...
bench --site dev.localhost run-tests --app notifypro

# Run specific test — تشغيل اختبار محدد
bench --site dev.localhost run-tests --app notifypro --module notifypro.tests.unit.test_my_service
```
...
- Update help files in `notifypro/help/` for DocType changes
